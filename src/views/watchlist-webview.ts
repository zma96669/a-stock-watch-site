...
<script nonce="${nonce}">
(() => { const vscode=acquireVsCodeApi(); let state=${initialState}; let structureKey=''; let modalAction; let marketCollapsed=false; let holdingsCollapsed=false; let menuCloseTimer; let dragPayload; let suppressClick=false; const $=s=>document.querySelector(s); const search=$('#search');
...
function structureSignature(value){return JSON.stringify({indices:value.indices.map(i=>[i.key,i.name]),groups:value.groups.map(g=>[g.id,g.name,g.sortOrder,g.collapsed]),entries:value.entries.map(e=>[e.code,e.name,e.groupId,e.sortOrder,e.followed,e.costPrice,e.shares])})}
function isHoldingEntry(entry){return Number(entry.shares)>0&&Number(entry.costPrice)>0}
function render(){structureKey=structureSignature(state);const app=$('#app');app.textContent='';app.appendChild(marketSection());if(!state.entries.length){const empty=document.createElement('div');empty.className='empty';empty.textContent='暂无自选股，搜索名称或代码后添加';app.appendChild(empty);updateDynamic();return}const holdings=state.entries.filter(isHoldingEntry);if(holdings.length)app.appendChild(groupSection('持仓','__holdings__',holdings,holdingsCollapsed,true));const followedGroup=state.groups.find(g=>g.id==='${DEFAULT_GROUP_ID}');if(followedGroup){const followed=state.entries.filter(x=>x.followed);app.appendChild(groupSection('我的关注',followedGroup.id,followed,followedGroup.collapsed,false,followedGroup,true))}state.groups.slice().sort((a,b)=>a.sortOrder-b.sortOrder).filter(g=>g.id!=='${DEFAULT_GROUP_ID}').forEach(g=>{const entries=state.entries.filter(x=>x.groupId===g.id&&!isHoldingEntry(x));app.appendChild(groupSection(g.name,g.id,entries,g.collapsed,false,g,false))});updateDynamic()}
...
function stockCard(entry,canDrag){const el=document.createElement('div');el.className='stock';el.dataset.code=entry.code;el.onclick=()=>{if(!suppressClick)vscode.postMessage({type:'select',code:entry.code})};el.innerHTML='<div class="stock-main"><span class="name"></span><span class="price"></span><span class="pct"></span><details class="stock-menu"><summary title="更多操作">···</summary><div class="stock-menu-panel"><select class="group-select" title="移动到分组"></select><button class="follow"></button><button class="edit">设置持仓</button><button class="remove">删除股票</button></div></details></div>';const menu=el.querySelector('.stock-menu');menu.onclick=e=>e.stopPropagation();menu.onmouseenter=cancelMenuClose;menu.onmouseleave=()=>scheduleMenuClose(menu);menu.ontoggle=()=>{if(menu.open)closeStockMenus(menu)};const groupSelect=el.querySelector('.group-select');state.groups.slice().sort((a,b)=>a.sortOrder-b.sortOrder).filter(g=>g.id!=='${DEFAULT_GROUP_ID}'||entry.groupId==='${DEFAULT_GROUP_ID}').forEach(g=>{const o=document.createElement('option');o.value=g.id;o.textContent=g.id==='${DEFAULT_GROUP_ID}'?'我的关注':g.name;o.selected=g.id===entry.groupId;groupSelect.appendChild(o)});groupSelect.onfocus=cancelMenuClose;groupSelect.onblur=()=>scheduleMenuClose(menu);groupSelect.onchange=e=>{e.stopPropagation();vscode.postMessage({type:'move',code:entry.code,groupId:groupSelect.value});menu.removeAttribute('open')};const follow=el.querySelector('.follow');follow.textContent=entry.followed?'取消关注':'关注';follow.onclick=e=>{e.stopPropagation();menu.removeAttribute('open');vscode.postMessage({type:entry.followed?'unfollow':'follow',code:entry.code})};el.querySelector('.edit').onclick=e=>{e.stopPropagation();menu.removeAttribute('open');editHolding(entry)};el.querySelector('.remove').onclick=e=>{e.stopPropagation();menu.removeAttribute('open');openDialog({title:'删除 '+entry.name+'？',confirmLabel:'删除',danger:true,submit:()=>vscode.postMessage({type:'remove',code:entry.code})})};if(canDrag)enableStockDrag(el,entry);return el}
