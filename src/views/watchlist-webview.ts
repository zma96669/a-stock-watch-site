...
function structureSignature(value){return JSON.stringify({groups:value.groups.map(g=>[g.id,g.name,g.sortOrder,g.collapsed]),entries:value.entries.map(e=>[e.code,e.name,e.groupId,e.sortOrder,e.followed,e.costPrice,e.shares])})}
...
function initialWatchlistMarkup(groups: readonly WatchlistGroup[], entries: readonly WatchlistEntry[]): string {
  ...
  return holdingsMarkup + followedMarkup + groups.slice().sort((a, b) => a.sortOrder - b.sortOrder).filter((group) => group.id !== DEFAULT_GROUP_ID).map((group) => {
