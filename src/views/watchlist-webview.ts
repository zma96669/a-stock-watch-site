...
import type { AlertRule, AlertSeverity, AlertRuleType, MarketSnapshot, StockRef, WatchlistEntry, WatchlistGroup } from '../domain/types';
...
<script nonce="${nonce}">
(() => { const vscode=acquireVsCodeApi(); let state=${initialState}; let structureKey=''; let modalAction; let marketCollapsed=false; let alertsCollapsed=true; let holdingsCollapsed=false; let menuCloseTimer; let dragPayload; let suppressClick=false; const $=s=>document.querySelector(s); const search=$('#search');
...
function configureAlert(entry){openDialog({title:'设置 '+entry.name+' 盘中提醒',fields:[{name:'ruleType',label:'提醒规则',value:'price-above',type:'select',options:[['price-above','价格突破'],['price-below','价格跌破'],['change-rise','涨幅达到'],['change-fall','跌幅达到'],['volume-ratio','量比达到'],['turnover','换手率达到'],['rapid-rise','3分钟快速上涨'],['rapid-fall','3分钟快速下跌']]},{name:'threshold',label:'阈值',value:'',type:'number'},{name:'severity',label:'提醒等级',value:'normal',type:'select',options:[['preview','预告'],['normal','普通'],['important','重要']]}],submit:v=>{if(!v.threshold.trim())return false;vscode.postMessage({type:'alertCreate',code:entry.code,ruleType:v.ruleType,threshold:v.threshold,severity:v.severity})}})}
