...
function stockCard(entry){const quote=state.snapshot.quotes[entry.code];const price=quote&&quote.price;const pct=quote&&quote.changePercent;const holding=Number(entry.shares)>0&&Number(entry.costPrice)>0;const el=document.createElement('div');el.className='stock '+(entry.code===state.currentCode?'current ':'')+(holding?'holding':'');el.onclick=()=>vscode.postMessage({type:'select',code:entry.code});const profit=holding&&price!=null?(price-entry.costPrice)*entry.shares:void 0;const profitPct=holding&&price!=null?(price-entry.costPrice)/entry.costPrice*100:void 0;el.innerHTML='<div class="stock-main"><span class="name"></span><span class="code"></span><span class="price"></span><span class="pct"></span><details class="stock-menu"><summary title="更多操作">···</summary><div class="stock-menu-panel"><select class="group-select" title="移动到分组"></select><button class="edit">设置持仓</button><button class="remove">删除股票</button></div></details></div>';el.querySelector('.name').textContent=quote?.name||entry.name;el.querySelector('.code').textContent=entry.code;el.querySelector('.price').textContent=fmt(price);setTone(el.querySelector('.price'),pct);el.querySelector('.pct').textContent=fmtPct(pct);setTone(el.querySelector('.pct'),pct);const tips=[(quote?.name||entry.name)+' '+entry.code,'最新 '+fmt(price)+'  涨跌 '+fmtPct(pct),'成交额 '+fmtMoney(quote?.amount)];if(holding){tips.push('成本 '+fmt(entry.costPrice)+'  股数 '+entry.shares);tips.push('持仓盈亏 '+fmtMoney(profit)+'  '+fmtPct(profitPct))}el.title=tips.join('\\n');const menu=el.querySelector('.stock-menu');menu.onclick=e=>e.stopPropagation();const groupSelect=el.querySelector('.group-select');state.groups.slice().sort((a,b)=>a.sortOrder-b.sortOrder).forEach(g=>{const o=document.createElement('option');o.value=g.id;o.textContent=g.name;o.selected=g.id===entry.groupId;groupSelect.appendChild(o)});groupSelect.onchange=e=>{e.stopPropagation();vscode.postMessage({type:'move',code:entry.code,groupId:groupSelect.value});menu.removeAttribute('open')};el.querySelector('.edit').onclick=e=>{e.stopPropagation();menu.removeAttribute('open');editHolding(entry)};el.querySelector('.remove').onclick=e=>{e.stopPropagation();menu.removeAttribute('open');if(confirm('从自选股移除 '+entry.name+'？'))vscode.postMessage({type:'remove',code:entry.code})};return el}
