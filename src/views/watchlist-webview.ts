...
<script nonce="${nonce}">
(() => { const vscode=acquireVsCodeApi(); let state={groups:[],entries:[],currentCode:void 0,snapshot:{quotes:{},stale:false}}; const $=s=>document.querySelector(s); const search=$('#search'); vscode.postMessage({type:'ready'});
...
window.addEventListener('message',e=>{const m=e.data||{};if(m.type==='state'){state=m.payload;render()}if(m.type==='searchResults')renderResults(m.rows||[]);if(m.type==='error'){const el=$('#error');el.textContent=m.message;el.classList.remove('hidden');setTimeout(()=>el.classList.add('hidden'),3500)}if(m.type==='focusSearch')search.focus()});
