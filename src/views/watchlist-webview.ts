...
<script nonce="${nonce}">
(() => { const vscode=acquireVsCodeApi(); let state=${initialState}; let structureKey=''; let modalAction; let holdingsCollapsed=false; let menuCloseTimer; const $=s=>document.querySelector(s); const search=$('#search');
...
function groupSection(name,id,entries,collapsed,isHoldings,group){const wrap=document.createElement('section');wrap.className='group'+(isHoldings?' holdings-group':'');wrap.dataset.groupId=id;if(isHoldings)wrap.dataset.holdings='true';const head=document.createElement('div');head.className='group-head';head.onclick=()=>{if(isHoldings){holdingsCollapsed=!holdingsCollapsed;renderSafely()}else vscode.postMessage({type:'toggleGroup',groupId:id})};head.innerHTML='<span class="tree-arrow">'+(collapsed?'▶':'▼')+'</span><span class="group-name"></span><span class="group-meta"></span><span class="group-actions">'+(isHoldings?'':'<button title="重命名">✎</button>')+'</span>';head.querySelector('.group-name').textContent=name;const action=head.querySelector('button');if(action)action.onclick=e=>{e.stopPropagation();openDialog({title:'重命名分组',fields:[{name:'name',label:'分组名称',value:group.name}],submit:v=>{if(v.name.trim())vscode.postMessage({type:'renameGroup',groupId:id,name:v.name})}})};wrap.appendChild(head);if(!collapsed)entries.forEach(entry=>wrap.appendChild(stockCard(entry)));return wrap}
