...
function groupSection(name,id,entries,collapsed,isHoldings,group,isFollowed=false){const wrap=document.createElement('section');wrap.className='group'+(isHoldings?' holdings-group':'');wrap.dataset.groupId=id;if(isHoldings)wrap.dataset.holdings='true';if(isFollowed)wrap.dataset.followed='true';const head=document.createElement('div');head.className='group-head';head.onclick=()=>{if(suppressClick)return;if(isHoldings){holdingsCollapsed=!holdingsCollapsed;renderSafely()}else vscode.postMessage({type:'toggleGroup',groupId:id})};const canRename=!isHoldings&&!isFollowed;head.innerHTML='<span class="tree-arrow">'+(collapsed?'▶':'▼')+'</span><span class="group-name"></span><span class="group-meta"></span><span class="group-actions">'+(canRename?'<button title="重命名">✎</button>':'')+'</span>';head.querySelector('.group-name').textContent=name;const action=head.querySelector('button');if(action)action.onclick=e=>{e.stopPropagation();openDialog({title:'重命名分组',fields:[{name:'name',label:'分组名称',value:group.name}],submit:v=>{if(v.name.trim())vscode.postMessage({type:'renameGroup',groupId:id,name:v.name})}})};if(!isHoldings&&!isFollowed)enableGroupDrag(head,id);wrap.appendChild(head);if(!collapsed)entries.forEach(entry=>wrap.appendChild(stockCard(entry,!isHoldings&&!isFollowed)));return wrap}
...
function initialWatchlistMarkup(groups: readonly WatchlistGroup[], entries: readonly WatchlistEntry[]): string {
  ...
  const followedMarkup = followedGroup ? `<section class="group"><div class="group-head"><span>${followedGroup.collapsed ? '▶' : '▼'}</span><span class="group-name">我的关注</span><span class="group-meta">${followed.length}只 --</span></div>${followedStocks}</section>` : '';
